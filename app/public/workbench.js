// workbench.js — the workbench, split verbatim out of app.js (phase 2):
// workSurface + Δ views, renderWB (skeleton, tab normalization, keep-alive
// carries), dividers, drag wiring, wireWB (editor overlay/caret/selection/
// pointer, suggestion decorations, composer), statusbar + usage meter, focus
// mode, pendingEdJump nav (texOpenAt/applyEdJump — co-located here because
// ESM forbids assigning an imported binding).
...
import {
  state, ui, hlStores, pdfPanes, tailBufs, snapFileCache, fileCache, drafts,
  edViews, draftBase, runBufs, htmlEdits, snapsCache, pendingPerms,
...
import {
  texProblemsFor, renderTexProblems, openSugs, sugStaleAt, sugPulsed, sugRedraw,
  resolveSug, markSugStale, armTexfixRetire, startTexFix, texForwardSearch,
...
import {
  ...
  syncViewerStack, mdPreviewSchedule,
} from './viewers.js';
import {
...
import {
  effectiveImpl as mpImpl, attachDock as mpAttachDock, parkHost as mpParkHost,
  onFallback as mpOnFallback, setFile as mpSetFile, requestSave as mpRequestSave,
...
/* Phase 3 S1 (A6 edge 13): a failed Monaco boot falls back atomically — the
   legacy editor re-renders with drafts intact; the stored editor:impl
   preference is never touched (session memory only, owned by monacoPane). */
mpOnFallback((key) => {
  toast('Monaco failed to load — using the built-in editor (details in the console)');
  if (key && state.projects[key]) renderWB(key);
});
...
function applyEdJump(root, ed) {
  if (!pendingEdJump || ed.dataset.fkey !== pendingEdJump.fkey) return;
  if (Date.now() - pendingEdJump.at > 15000) { pendingEdJump = null; return; } // stale
  const { line, col } = pendingEdJump;
  pendingEdJump = null;
  const nl = ed.value.split('\n');
  const ln = Math.min(Math.max(1, line), nl.length);
  let pos = 0;
  for (let i = 0; i < ln - 1; i++) pos += nl[i].length + 1;
  pos += Math.min(Math.max(0, col - 1), nl[ln - 1].length);
  ed.focus();
  try { ed.setSelectionRange(pos, pos); } catch { /* stale */ }
  const lh = parseFloat(getComputedStyle(ed).lineHeight) || 19;
  // soft-wrapped files have variable line heights — the overlay's line span
  // knows where line N really starts; grid math covers the pre-paint case
  const lnEl = root.querySelector('#codeHL')?.children?.[ln - 1];
  ed.scrollTop = Math.max(0, (lnEl ? lnEl.offsetTop : (ln - 1) * lh) - ed.clientHeight * 0.35);
  const hlPre = root.querySelector('.codeHL');
  if (hlPre) {
    hlPre.scrollTop = ed.scrollTop;
    texJumpFlash(root.querySelector('#codeHL'), ln);
  }
}
...
function workSurface(key, task, files, fi) {
  ...
  if (fi === 'snaps') {
    ...
    if (rel && c && !c.loading && !c.error && !c.truncated) {
      ...
      // prose files soft-wrap to the pane width (one line number per logical
      // line, no horizontal scroll); code files keep wrap="off" (1 line = 1 row)
      const softWrap = isTex && hlOn;
      if (mpImpl() === 'monaco') {
        // Phase 3 S1 (frozen seam, A7): the monaco path renders a PLACEHOLDER
        // slot only — the live editor is the kept #mpHost inside this
        // workbench's persistent .mdock (a build-once sibling of the refilled
        // slots), positioned over this slot by monacoPane.syncDock after
        // wiring. The host must never render inside this innerHTML'd subtree.
        body = `<div class="monacoSlot" id="monacoSlot" data-fkey="${esc(fkey)}" data-rel="${esc(rel)}" data-ext="${esc(ext)}"><div class="mpBooting">⟐ Monaco editor loading…</div></div>`;
      } else {
        const ta = `<textarea id="codeEditor" class="codeEdit${hlOn ? ' codeSrc' : ''}" data-fkey="${esc(fkey)}" data-rel="${esc(rel)}" data-ext="${esc(ext)}"
        spellcheck="false" autocomplete="off" autocapitalize="off" wrap="${softWrap ? 'soft' : 'off'}">${esc(dirty ? drafts[fkey] : c.text)}</textarea>`;
        ...
        // colors (wired in wireWB); unknown ext → a plain textarea. Both editable
        // modes now ride inside .edWrap with a line-number gutter (filled + synced
        // in wireWB); the gutter offsets the content via --gutW.
        const gutter = `<div class="edGutter"><div class="edGutterInner" id="edGutterInner"></div></div>`;
        body = hlOn
          ? `<div class="edWrap${softWrap ? ' softWrap' : ''}">${gutter}<pre class="codeEdit codeHL" aria-hidden="true"><code id="codeHL"></code></pre>${ta}</div>`
          : `<div class="edWrap edPlain">${gutter}${ta}</div>`;
      }
      const texBits = isTex
...
export function renderWB(key) {
  ...
  // textarea-scoped: the kept Monaco host shares the #codeEditor id (P14) but
  // owns its own viewStates map — it must never feed this legacy capture.
  const prevEd = root.querySelector('textarea#codeEditor');
  ...
  // …and bank it per file, so the place ALSO survives the editor unmounting
  // entirely (≋ console tab and back, or another file's tab in between)
  if (edState && edState.fkey) {
    edViews[edState.fkey] = {
      scrollTop: edState.scrollTop, scrollLeft: edState.scrollLeft,
      selStart: edState.selStart, selEnd: edState.selEnd,
    };
    const ks = Object.keys(edViews); // light cap — oldest entry out
    if (ks.length > 50) delete edViews[ks[0] === edState.fkey ? ks[1] : ks[0]];
  }
  // a pending line jump must win over a remembered viewport below
  const jumpFkey = pendingEdJump ? pendingEdJump.fkey : null;
  ...
  // the kept editor is a singleton, the class is per-view DOM. Legacy path:
  // never called (byte-identical behavior).
  if (mpImpl() === 'monaco' && ui.view === key) mpHoldEditor(editHold);
  ...
  {
    const ed = root.querySelector('textarea#codeEditor');
    if (ed && edState && ed.dataset.fkey === edState.fkey && ed.dataset.fkey !== jumpFkey) {
      // same file re-rendered in place — exact continuation, focus included.
      // (jumpFkey: wireWB already landed a problems-click/inverse-sync jump
      // on this editor — the jump's caret and scroll must stand, not the
      // pre-click state captured above.)
      // ORDER MATTERS: focusing a textarea scrolls it to its caret, and a
      // freshly-built node's caret sits at 0 — so place the caret first,
      // focus second, and restore the viewport LAST. The old order (viewport,
      // focus, caret) let focus() zero the scroll on every background
      // re-render: the "bumped to the top while typing" bug
      // (docs/editor-scroll-reset-diagnosis.txt).
      try { ed.setSelectionRange(edState.selStart, edState.selEnd); } catch { /* ranges can go stale */ }
      if (edState.focused) ed.focus({ preventScroll: true });
      ed.scrollTop = edState.scrollTop;
      ed.scrollLeft = edState.scrollLeft;
      ed._hlSync?.(); // pan the overlay NOW — the scroll event is async and loses to queued clicks (click-race)
    } else if (ed && edViews[ed.dataset.fkey] && ed.dataset.fkey !== jumpFkey) {
      // a file coming BACK on screen (console round-trip, tab switch,
      // project switch) — restore its remembered place. No focus steal
      // (the user clicked a tab, not the text), and a pending line jump
      // (problems click / inverse sync) beats the memory.
      const saved = edViews[ed.dataset.fkey];
      try { ed.setSelectionRange(saved.selStart, saved.selEnd); } catch { /* stale */ }
      ed.scrollTop = saved.scrollTop;
      ed.scrollLeft = saved.scrollLeft;
      ed._hlSync?.(); // same immediate pan as the exact-continuation branch
    }
  }
  if (pinFocused) {
...
function wireWB(root, key, task, files) {
  ...
  // textarea-scoped on purpose: in monaco mode the kept host div carries the
  // #codeEditor id (P14) and must never enter this legacy wiring block
  const ed = root.querySelector('textarea#codeEditor');
  if (ed) {
    const k = ed.dataset.fkey;
    // ── line-number gutter (both editable modes) ──
    // wrap="off" files: 1 line = 1 row. softWrap files: cells get per-line
    // heights from syncGutterHts so number N spans line N's wrapped rows.
    // The gutter is a fixed left column; the content is offset right by --gutW.
    // It vertical-scroll-syncs via a translateY (programmatic scrollTop restores
    // fire a scroll event, so a restored scroll position self-aligns).
    const gutterInner = root.querySelector('#edGutterInner');
    const edWrapEl = ed.closest('.edWrap');
    const softWrapOn = !!edWrapEl?.classList.contains('softWrap');
    // syntax-color overlay (only present for known languages) — declared before
    // the gutter because the numbers key off ITS scroll position: the overlay
    // is the layer the user reads, and under soft wrap its scroll range can
    // differ from the textarea's (Chrome wraps a textarea at a slightly
    // different effective width than the pixel-identical pre — see
    // docs/editor-click-offset-diagnosis.txt), so raw ed.scrollTop would let
    // the numbers sail on while the visible text sat clamped at its own end.
    const hlCode = root.querySelector('#codeHL');
    const hlPre = hlCode ? hlCode.parentElement : null;
    let gutHts = []; // per-line gutter cell heights (softWrap only; px)
    let gutterScroll = null;
    if (gutterInner && edWrapEl) {
      let gutLnCount = -1;
      const renderGutter = () => {
        const n = (ed.value.match(/\n/g) || []).length + 1;
        if (n === gutLnCount) return;         // only rebuild when the line count moves
        gutLnCount = n;
        gutHts = [];                          // fresh rows carry no stale heights
        edWrapEl.style.setProperty('--gutW', (Math.max(2, String(n).length) * 7 + 15) + 'px');
        let html = '';
      ...
      gutterScroll = () => {
        let st = ed.scrollTop;
        if (hlPre) { hlPre.scrollTop = st; st = hlPre.scrollTop; } // clamped = what's painted
        gutterInner.style.transform = `translateY(${-st}px)`;
      };
      renderGutter();
      gutterScroll();
      // overlay files scroll-sync through hlSync (which ends in gutterScroll);
      // plain files drive the gutter straight off the textarea
      if (!hlPre) ed.addEventListener('scroll', gutterScroll);
      ed.addEventListener('input', renderGutter);
    }
    // ── scroll-range parity (softWrap) ──
    // Diverged wrap columns also mean diverged TOTAL heights, so one layer
    // runs out of scroll before the other: the textarea (the scroll surface)
    // either sails past the overlay's end — the old misaligned-numbers bug —
    // or stops short of its last rows, leaving them unreachable. Pad the
    // shorter layer's bottom to the taller one after every repaint: both
    // ranges stay equal, scrolling is 1:1 down to the last row, and the
    // clamp in gutterScroll never has to engage.
    const edPadB = (softWrapOn && hlPre) ? (parseFloat(getComputedStyle(ed).paddingBottom) || 0) : 0;
    let padEd = 0, padHl = 0; // extra px currently added to each layer
    const syncScrollRange = () => {
      if (!softWrapOn || !hlPre) return;
      const d = (hlPre.scrollHeight - padHl) - (ed.scrollHeight - padEd);
      const wantEd = Math.max(0, d), wantHl = Math.max(0, -d);
      if (wantEd !== padEd) { padEd = wantEd; ed.style.paddingBottom = (edPadB + wantEd) + 'px'; }
      if (wantHl !== padHl) { padHl = wantHl; hlPre.style.paddingBottom = (edPadB + wantHl) + 'px'; }
    };
    const hlSync = () => {
      if (!hlPre) return;
      hlPre.scrollTop = ed.scrollTop;
      hlPre.scrollLeft = ed.scrollLeft;
      if (gutterScroll) gutterScroll(); // reads back the clamped position
      // the suggestion layer lives above the textarea (clicks!) in content
      // coordinates — counter-scroll it so chips ride with their lines
      if (sugLayer) sugLayer.style.transform = `translate(${-ed.scrollLeft}px, ${-hlPre.scrollTop}px)`;
      if (syncSel) syncSel(); // selection bands are viewport-windowed
    };
    // renderWB's viewport restore reaches this through the node: assigning
    // ed.scrollTop fires 'scroll' ASYNCHRONOUSLY, and queued input is
    // delivered ahead of it — until then a just-painted overlay would show
    // (and hit-test as) the top of the file while the textarea is already
    // back at the user's place (docs/editor-click-race-diagnosis.txt)
    ed._hlSync = hlSync;
    const isTex = TEX_EXTS.includes(ed.dataset.ext);
    let hlQueued = false;
    let syncCaret = null;  // custom caret + current-line tint (set up below)
    let syncSel = null;    // custom selection bands (set up below)
    let renderSugs = null; // Claude-fix suggestion decorations (set up below)
    let sugLayer = null;   // interactive chips/cards — counter-scrolled in hlSync
    // ── soft-wrap gutter heights ──
    // With wrap="soft" a logical line can span several visual rows. The painted
    // overlay is the ground truth for where lines wrap (.ln span i IS line i,
    // laid out with the textarea's exact metrics), so each gutter cell takes
    // its height from the overlay — number N spans all of line N's rows.
    const syncGutterHts = () => {
      if (!softWrapOn || !hlCode || !gutterInner) return;
      const lns = hlCode.children, rows = gutterInner.children;
      const n = Math.min(lns.length, rows.length);
      if (!n) return;
      const tops = new Array(n);
      for (let i = 0; i < n; i++) tops[i] = lns[i].offsetTop; // one layout pass
      const padB = parseFloat(getComputedStyle(hlPre).paddingBottom) || 0;
      const lastH = Math.max(0, hlPre.scrollHeight - padB - tops[n - 1]);
      for (let i = 0; i < n; i++) {
        const h = i < n - 1 ? tops[i + 1] - tops[i] : lastH;
        if (gutHts[i] !== h) { gutHts[i] = h; rows[i].style.height = h + 'px'; }
      }
    };
    const hlPaint = () => {
      if (!hlCode) return;
      ...
      // repaints rebuild the line spans — restore the problem tints + caret line
      if (isTex) texMarkLines(hlCode, texProblemsFor(key), ed.dataset.rel);
      syncScrollRange(); // rewrapped content can move the layers' total heights
      syncGutterHts();
      if (syncCaret) syncCaret();
      if (syncSel) syncSel();
      if (renderSugs) renderSugs();
      hlSync();
    };
    ...
    if (softWrapOn && hlCode && typeof ResizeObserver === 'function') {
      // a pane resize rewraps every line — remeasure heights + decorations
      let roQueued = false;
      const ro = new ResizeObserver(() => {
        ...
        requestAnimationFrame(() => {
          roQueued = false;
          if (!ed.isConnected) { ro.disconnect(); return; }
          gutHts = [];
          syncScrollRange(); // a rewrap moves the layers' total heights
          syncGutterHts();
          hlSync();          // …and where the clamp lands — re-align the numbers
          if (syncCaret) syncCaret();
          if (syncSel) syncSel();
          if (renderSugs) renderSugs();
        });
    ...
    if (hlCode) {
      ...
      // ── custom caret + current-line tint ──
      // The native caret is drawn font-height inside the 1.75 line box (it
      // floats in the leading) and 1px thin. The overlay shares exact metrics,
      // so draw a line-tall 2px caret there instead and tint the active line.
      const caret = document.createElement('div');
      caret.className = 'edCaret';
      hlPre.appendChild(caret);
      // current-line band — a full-width tint behind the glyphs spanning ALL
      // of the logical line's wrapped rows (an .ln background can't: inline
      // spans only paint under their own glyph boxes, so the tint used to end
      // ragged at each row's last character). Inserted BEFORE the code element
      // so the text, problem tints, and selection bands all paint above it.
      const lnBand = document.createElement('div');
      lnBand.className = 'edLnBand';
      lnBand.style.display = 'none';
      hlPre.insertBefore(lnBand, hlCode);
      ...
      const caretX = (prefix) => {
        // expand tabs to their 4-wide stops, then measure the real glyphs so
        // wide unicode (α, ⌘) can't drift the bar off its column
        let vis = '';
        for (const ch of prefix) {
          if (ch === '\t') vis += ' '.repeat(4 - (vis.length % 4));
          else vis += ch;
        }
        return padL + mctx.measureText(vis).width;
      };
      // ── wrap-aware measurement (softWrap files) ──
      // With wrap="soft", line×lineHeight and col×charW stop being true: a
      // logical line spans several visual rows. The overlay's .ln spans hold
      // the exact laid-out text, so positions are measured off them with
      // Ranges. rowRects(line, c0, c1) → merged per-visual-row rects in the
      // pre's content coordinates; [] pre-paint (callers fall back to grid).
      const nodePt = (el, col) => { // text node + offset at column `col` of a line el
        const w = document.createTreeWalker(el, NodeFilter.SHOW_TEXT);
        let nd, seen = 0;
        while ((nd = w.nextNode())) {
          if (seen + nd.data.length >= col) return [nd, col - seen];
          seen += nd.data.length;
        }
        return null;
      };
      const rowRects = (li, c0, c1) => {
        const el = hlCode.children[li];
        if (!el) return [];
        const a = nodePt(el, c0), b = nodePt(el, Math.max(c0, c1));
        if (!a || !b) return [];
        const rg = document.createRange();
        try { rg.setStart(a[0], a[1]); rg.setEnd(b[0], b[1]); } catch { return []; }
        const pr = hlPre.getBoundingClientRect();
        const out = [];
        for (const rc of rg.getClientRects()) {
          if (!rc.width && !rc.height) continue;
          // a Range measures the GLYPH box (~font-size tall, centered in the
          // 1.75 line box by half-leading), and inline spans report offsetTop
          // at the glyph box too — but every visual row is exactly lh tall, so
          // snap to the GLOBAL row grid (origin padT) and bands/carets/marks
          // span full rows exactly like the legacy line×lh math
          const row = Math.round((rc.top - pr.top + hlPre.scrollTop - padT) / lh - 0.5);
          const top = padT + Math.max(0, row) * lh;
          const prev = out[out.length - 1];
          if (prev && prev.top === top) { // same visual row → merge token fragments
            prev.x0 = Math.min(prev.x0, rc.left - pr.left + hlPre.scrollLeft);
            prev.x1 = Math.max(prev.x1, rc.right - pr.left + hlPre.scrollLeft);
          } else {
            out.push({
              top,
              x0: rc.left - pr.left + hlPre.scrollLeft,
              x1: rc.right - pr.left + hlPre.scrollLeft,
            });
          }
        }
        return out;
      };
      // caret point for (logical line, text before the caret on that line) —
      // measures the char AT the caret, so a position past a soft-wrap point
      // correctly lands at the start of the next visual row
      const caretPt = (line, rowPre) => {
        if (softWrapOn) {
          const r = rowRects(line, rowPre.length, rowPre.length + 1);
          if (r.length) return { x: r[0].x0, y: r[0].top };
          // EOL: a Range over just the newline can collapse to no rects —
          // anchor to the RIGHT edge of the char before the caret instead
          if (rowPre.length) {
            const rb = rowRects(line, rowPre.length - 1, rowPre.length);
            if (rb.length) { const lastR = rb[rb.length - 1]; return { x: lastR.x1, y: lastR.top }; }
          }
          // blank line / unmeasurable: the line element's own layout position
          // — NEVER the legacy line×lh grid below, which ignores every wrap
          // above and would paint the caret rows away
          const el2 = hlCode.children[line];
          if (el2) {
            const row = Math.max(0, Math.round((el2.offsetTop - padT) / lh - 0.5));
            return { x: padL, y: padT + row * lh };
          }
        }
        return { x: caretX(rowPre), y: padT + line * lh };
      };
      // wrap-aware caret point for texEditor's completion popup (same
      // convention as its internal fallback: textarea-relative, minus scroll)
      ed._caretXY = () => {
        const upto = ed.value.slice(0, ed.selectionStart);
        const line = (upto.match(/\n/g) || []).length;
        const rowPre = upto.slice(upto.lastIndexOf('\n') + 1);
        const p = caretPt(line, rowPre);
        return { x: p.x - ed.scrollLeft, y: p.y + lh - ed.scrollTop };
      };
      // the logical line's full extent on the row grid — every wrapped row,
      // top snapped exactly like caretPt/rowRects snap (so band, caret, and
      // gutter cells can never disagree on where a line starts)
      const lineExtent = (line) => {
        if (!softWrapOn) return { top: padT + line * lh, h: lh };
        const el = hlCode.children[line];
        if (!el) return null; // pre-paint — no geometry yet
        const snap = (y) => Math.max(0, Math.round((y - padT) / lh - 0.5));
        const r0 = snap(el.offsetTop);
        const nx = hlCode.children[line + 1];
        let r1;
        if (nx) {
          r1 = snap(nx.offsetTop);
        } else { // last line: content end (minus the pad syncScrollRange added)
          const padB = parseFloat(getComputedStyle(hlPre).paddingBottom) || 0;
          r1 = Math.round((hlPre.scrollHeight - padB - padT) / lh);
        }
        return { top: padT + r0 * lh, h: Math.max(lh, (r1 - r0) * lh) };
      };
      caret.style.height = `${lh}px`;
      syncCaret = () => {
        const active = document.activeElement === ed && ed.selectionStart === ed.selectionEnd;
        if (!active) {
        ...
        const upto = ed.value.slice(0, ed.selectionStart);
        const line = (upto.match(/\n/g) || []).length;
        const p = caretPt(line, upto.slice(upto.lastIndexOf('\n') + 1));
        caret.style.display = '';
        caret.style.top = `${p.y}px`;
        caret.style.left = `${p.x - 1}px`;
        caret.style.animation = 'none'; // hold it solid while it moves
        void caret.offsetWidth;
        caret.style.animation = '';
        const b = lineExtent(line);
        if (b) {
          lnBand.style.display = '';
          lnBand.style.top = `${b.top}px`;
          lnBand.style.height = `${b.h}px`;
          // span the full scrollable width so wrap="off" files stay tinted
          // when scrolled sideways (softWrap: scrollWidth == clientWidth)
          lnBand.style.width = `${hlPre.scrollWidth}px`;
        } else lnBand.style.display = 'none';
      };
      // ── custom selection bands ──
      // Same disease as the native caret: ::selection is painted by the
      // TEXTAREA layer, and at some zoom/display-scaling combinations Chrome
      // lands its textarea rows a few px off the overlay glyphs — the band
      // read as off-center (2026-07-20 screenshots; unreproducible at 1:1
      // scaling, where both layers agree to the pixel). Same cure: the native
      // band is transparent in CSS, and rows are drawn HERE with the caret's
      // own coordinate math. Windowed to the viewport so select-all on a
      // huge file stays cheap; bands live in the scrolled pre, so they ride
      // scroll for free.
      const selBands = document.createElement('div');
      selBands.className = 'selBands';
      hlPre.appendChild(selBands);
      syncSel = () => {
        const s = ed.selectionStart, e2 = ed.selectionEnd;
        if (document.activeElement !== ed || s === e2) {
          if (selBands.firstChild) selBands.replaceChildren();
          return;
        }
        const upS = ed.value.slice(0, s), upE = ed.value.slice(0, e2);
        const sL = (upS.match(/\n/g) || []).length;
        const eL = (upE.match(/\n/g) || []).length;
        const sPre = upS.slice(upS.lastIndexOf('\n') + 1);
        const ePre = upE.slice(upE.lastIndexOf('\n') + 1);
        const lines = ed.value.split('\n');
        let v0, v1;
        if (softWrapOn && hlCode.children.length) {
          // variable line heights → window by the overlay's line offsets
          const kids = hlCode.children;
          const lineAtY = (y) => {
            let lo = 0, hi = kids.length - 1, ans = 0;
            while (lo <= hi) {
              const m = (lo + hi) >> 1;
              if (kids[m].offsetTop <= y) { ans = m; lo = m + 1; } else hi = m - 1;
            }
            return ans;
          };
          v0 = Math.max(sL, lineAtY(ed.scrollTop) - 1);
          v1 = Math.min(eL, lineAtY(ed.scrollTop + ed.clientHeight) + 1);
        } else {
          v0 = Math.max(sL, Math.floor((ed.scrollTop - padT) / lh) - 2);
          v1 = Math.min(eL, Math.floor((ed.scrollTop + ed.clientHeight - padT) / lh) + 2);
        }
        const frag = document.createDocumentFragment();
        const band = (top, left, width) => {
          const b = document.createElement('div');
          b.className = 'selBand';
          b.style.top = `${top}px`;
          b.style.left = `${left}px`;
          b.style.width = `${Math.max(3, width)}px`;
          b.style.height = `${lh}px`;
          frag.appendChild(b);
        };
        for (let ln = v0; ln <= v1; ln++) {
          if (softWrapOn) {
            const c0 = ln === sL ? sPre.length : 0;
            const c1 = ln === eL ? ePre.length : (lines[ln] ?? '').length + 1;
            const rrs = rowRects(ln, c0, c1);
            if (rrs.length) {
              // the newline nub rides the LAST visual row of a full line
              rrs.forEach((rr, i) => band(rr.top, rr.x0,
                rr.x1 - rr.x0 + (ln !== eL && i === rrs.length - 1 ? 7 : 0)));
              continue;
            }
          }
          const x0 = ln === sL ? caretX(sPre) : padL;
          // a fully-selected line includes its newline — the small nub past
          // the last glyph says so (mirrors the native rendering)
          const x1 = ln === eL ? caretX(ePre) : caretX(lines[ln] ?? '') + 7;
          band(padT + ln * lh, x0, x1 - x0);
        }
        selBands.replaceChildren(frag);
      };

      const caretSoon = () => requestAnimationFrame(() => { syncCaret(); syncSel(); });
      // 'selectionchange' on text controls covers arrows/mouse/programmatic
      // moves in current Chrome; the others are belt-and-braces
      ['selectionchange', 'focus', 'blur', 'keyup', 'click', 'input'].forEach((ev) =>
        ed.addEventListener(ev, caretSoon));

      // ── overlay-authoritative pointer input (softWrap files) ──
      // Chrome's textarea soft-wraps at a slightly DIFFERENT effective width
      // than the pixel-identical overlay pre — the delta depends on the
      // pane's fractional width, so no static CSS parity exists
      // (docs/editor-click-offset-diagnosis.txt). The user aims at the
      // OVERLAY's glyphs, so pointer input is hit-tested against the overlay:
      // one caretPositionFromPoint call with the layers' pointer-events
      // flipped, mapped to (line, col), driven into the textarea with
      // setSelectionRange — the textarea's private (diverging) layout never
      // sees the gesture. Unmappable points (pre-paint, decorations) fall
      // back to the native path.
      if (softWrapOn) {
        // Pure overlay geometry — deliberately NOT caretPositionFromPoint:
        // that API's hit resolution is not coordinate-consistent under
        // zoom/display scaling, and any miss used to fall back to the NATIVE
        // hit test, re-admitting the divergence bug exactly at line ends and
        // beside equations. Here the click is resolved with the SAME rulers
        // the caret paint uses (viewport rects of the overlay's own text), so
        // input and paint cannot disagree — under any zoom, DPR, or layer
        // divergence. Everything is measured in viewport CSS px.
        const glyphRect = (lnEl, col) => { // rect of the char at col, or null
          const w2 = document.createTreeWalker(lnEl, NodeFilter.SHOW_TEXT);
          let nd2, seen = 0;
          while ((nd2 = w2.nextNode())) {
            if (seen + nd2.data.length > col) {
              const rg = document.createRange();
              try { rg.setStart(nd2, col - seen); rg.setEnd(nd2, col - seen + 1); } catch { return null; }
              for (const rc of rg.getClientRects()) if (rc.width || rc.height) return rc;
              return null;
            }
            seen += nd2.data.length;
          }
          return null;
        };
        const posFromPoint = (cx, cy) => {
          const kids = hlCode.children;
          if (!kids.length) return null; // pre-paint — native is all there is
          // Mid-rebuild the layers can disagree: a build transition re-renders
          // the stack, hlPaint runs at mount (pan 0), the viewport restore
          // then moves the TEXTAREA — and Chrome delivers a queued click
          // ahead of the async scroll re-sync, so the geometry here would be
          // the top of the file while the user aimed at their deep view. The
          // textarea's scroll IS that view — re-align before resolving, or
          // the caret (and the next keystrokes) land hundreds of lines off
          // (docs/editor-click-race-diagnosis.txt).
          if (Math.abs(hlPre.scrollTop - ed.scrollTop) > 1
            || Math.abs(hlPre.scrollLeft - ed.scrollLeft) > 1) hlSync();
          // 1. the logical line: binary search the .ln spans' viewport tops
          let lo = 0, hi = kids.length - 1, line = 0;
          while (lo <= hi) {
            const m = (lo + hi) >> 1;
            if (kids[m].getBoundingClientRect().top <= cy) { line = m; lo = m + 1; } else hi = m - 1;
          }
          const lnEl = kids[line];
          const text = lnEl.textContent.replace(/\n$/, '');
          if (!text.length) return { line, col: 0 }; // blank line
          // 2. the visual row: the whole line's Range rects, one per row
          const rows = [];
          {
            const w2 = document.createTreeWalker(lnEl, NodeFilter.SHOW_TEXT);
            let nd2, first = null, last = null, seen = 0, endPt = null;
            while ((nd2 = w2.nextNode())) {
              if (!first) first = nd2;
              if (seen + nd2.data.length >= text.length) { endPt = [nd2, text.length - seen]; break; }
              seen += nd2.data.length;
              last = nd2;
            }
            if (!endPt) endPt = [last || first, (last || first).data.length];
            const rg = document.createRange();
            ...
            for (const rc of rg.getClientRects()) {
              ...
              if (prev && Math.abs(rc.top - prev.top) < rc.height / 2) {
                prev.x0 = Math.min(prev.x0, rc.left); prev.x1 = Math.max(prev.x1, rc.right);
              } else rows.push({ top: rc.top, bottom: rc.bottom, x0: rc.left, x1: rc.right });
            }
          }
          if (!rows.length) return { line, col: 0 };
          let ri = rows.length - 1;
          for (let i = 0; i < rows.length; i++) if (cy < rows[i].bottom) { ri = i; break; }
          const row = rows[ri];
          // 3. the column: binary search chars — rows before ri are all
          // "before", then within the row compare glyph centers to x for
          // native nearest-boundary rounding
          const before = (col) => { // is the caret boundary at `col` left/above the point?
            const rc = glyphRect(lnEl, col);
            if (!rc) return true; // unmeasurable (shouldn't happen mid-text) — bias left
            if (rc.bottom <= row.top + 1) return true;   // earlier visual row
            if (rc.top >= row.bottom - 1) return false;  // later visual row
            return (rc.left + rc.right) / 2 <= cx;       // same row: nearest boundary
          };
          let a = 0, b = text.length; // invariant: boundary a is before, b is not (or end)
          while (a < b) {
            const m = (a + b) >> 1;
            if (before(m)) a = m + 1; else b = m;
          }
          return { line, col: a };
        };
        // native-ish word bounds: a run of word chars, else of symbols, else of spaces
        const wordAt = (text, i) => {
          if (!text.length) return [0, 0];
          if (i >= text.length) i = text.length - 1;
          const cls = (ch) => (/\w/.test(ch) ? 1 : /\s/.test(ch) ? 2 : 3);
          const c = cls(text[i]);
          let a = i, b = i + 1;
          while (a > 0 && text[a - 1] !== '\n' && cls(text[a - 1]) === c) a--;
        ...
        let drag = null; // { starts, lens, anchorA, anchorB } for the active gesture
        // PointerEvent.detail is 0 on pointerdown (and the native chording is
        // suppressed with the default action anyway) — count clicks ourselves.
        // A chord is an UNINTERRUPTED pointer gesture: any keystroke between
        // clicks ends it (click → type → quick same-spot click must not read
        // as a double-click).
        let chord = { t: 0, x: 0, y: 0, n: 0 };
        ed.addEventListener('keydown', () => { chord.t = 0; });
        const offsetOf = (p) => {
          const line = Math.max(0, Math.min(p.line, drag.lens.length - 1));
          return drag.starts[line] + Math.min(p.col, drag.lens[line]);
        };
        const applySel = (off) => {
          const a = Math.min(drag.anchorA, off), b = Math.max(drag.anchorB, off);
          ed.setSelectionRange(a, b, off < drag.anchorA ? 'backward' : 'forward');
          caretSoon();
        };
        ed.addEventListener('pointerdown', (e) => {
          if (e.button !== 0 || e.pointerType === 'touch') return;
          const p = posFromPoint(e.clientX, e.clientY);
          if (!p) return; // let the native gesture handle what we can't map
          e.preventDefault(); // the textarea's own hit test must not run
          if (document.activeElement !== ed) ed.focus({ preventScroll: true });
          // wall-clock, NOT e.timeStamp: synthesized events (tests, tooling)
          // can carry equal timestamps, which would chord unrelated clicks at
          // the same spot into a phantom double-click
          const now = performance.now();
          chord.n = (now - chord.t < 450
            && Math.abs(e.clientX - chord.x) < 5 && Math.abs(e.clientY - chord.y) < 5)
            ? chord.n + 1 : 1;
          chord.t = now; chord.x = e.clientX; chord.y = e.clientY;
          const lens = ed.value.split('\n').map((l) => l.length);
          const starts = []; let acc = 0;
          for (const n of lens) { starts.push(acc); acc += n + 1; }
          drag = { starts, lens, anchorA: 0, anchorB: 0 };
          const off = offsetOf(p);
          if (chord.n >= 3) { // line select, newline included (as native does)
            const ln = Math.min(p.line, lens.length - 1);
            drag.anchorA = starts[ln];
            drag.anchorB = Math.min(starts[ln] + lens[ln] + 1, ed.value.length);
          } else if (chord.n === 2) {
            const [a, b] = wordAt(ed.value, off);
            drag.anchorA = a; drag.anchorB = b;
          } else if (e.shiftKey) { // extend from the fixed end of the current selection
            drag.anchorA = drag.anchorB = ed.selectionDirection === 'backward'
              ? ed.selectionEnd : ed.selectionStart;
          } else {
            drag.anchorA = drag.anchorB = off;
          }
          applySel(off);
          try { ed.setPointerCapture(e.pointerId); } catch { /* detached mid-gesture */ }
        });
        ed.addEventListener('pointermove', (e) => {
          if (!drag) return;
          // nudge-scroll when dragging past the box (keep the pointer moving
          // to keep scrolling — no hold-timer, deliberately simple)
          const er = ed.getBoundingClientRect();
          if (e.clientY < er.top) ed.scrollTop -= Math.min(40, er.top - e.clientY);
          else if (e.clientY > er.bottom) ed.scrollTop += Math.min(40, e.clientY - er.bottom);
          const p = posFromPoint(e.clientX, e.clientY);
          if (p) applySel(offsetOf(p));
        });
      ...
      // ── Claude-fix suggestion decorations ──
      // String-anchored: each open suggestion's find-text is located in the
      // LIVE buffer, so the highlight follows the code no matter how much has
      // been typed since Claude was dispatched. Marks paint in the overlay
      // (scroll for free, behind the glyphs); the interactive ✦ chips + card
      // live in a counter-scrolled layer above the textarea.
      if (isTex) {
        const wrap = ed.closest('.edWrap');
        const clip = document.createElement('div');
        clip.className = 'sugClip';
        sugLayer = document.createElement('div');
        sugLayer.className = 'sugLayer';
        clip.appendChild(sugLayer);
        wrap.appendChild(clip);

        const sugAt = (s) => ed.value.indexOf(s.find);
        const lineColOf = (at) => {
          const pre2 = ed.value.slice(0, at);
          return { line: (pre2.match(/\n/g) || []).length, prefix: pre2.slice(pre2.lastIndexOf('\n') + 1) };
        };

        const applySug = (s) => {
          const at = sugAt(s);
          if (at < 0) { resolveSug(key, s.id, 'stale'); return; }
          ed.setRangeText(s.replace, at, at + s.find.length, 'end');
          ed.dispatchEvent(new Event('input', { bubbles: true }));
          ed.focus();
          const ln = ed.value.slice(0, at).split('\n').length;
          requestAnimationFrame(() => texJumpFlash(hlCode, ln));
          resolveSug(key, s.id, 'accepted');
        };

        renderSugs = () => {
          if (!ed.isConnected) return; // superseded by a newer render
          hlPre.querySelectorAll('.sugMark').forEach((n) => n.remove());
          sugLayer.querySelectorAll('.sugPane').forEach((n) => n.remove());
          const rel = ed.dataset.rel;
          for (const s of openSugs(key)) {
            if (s.file !== rel) continue;
            const at = sugAt(s);
            if (at < 0) { markSugStale(key, s); continue; }
            delete sugStaleAt[s.id];
            const { line, prefix } = lineColOf(at);
            const fresh = !sugPulsed.has(s.id);
            if (fresh) sugPulsed.add(s.id);
            const segs = s.find.split('\n').slice(0, 6);
            let segPrefix = prefix;
            let paneX = null, paneY = null;
            const mkMark = (top, left, width) => {
              const mark = document.createElement('div');
              mark.className = 'sugMark' + (fresh ? ' pulse' : '');
              mark.style.top = `${top}px`;
              mark.style.left = `${left}px`;
              mark.style.width = `${width}px`;
              mark.style.height = `${lh}px`;
              hlPre.appendChild(mark);
            };
            segs.forEach((seg, i) => {
              // soft wrap: a segment can span several visual rows — one mark
              // per row, measured off the overlay; grid math otherwise
              const rrs = softWrapOn
                ? rowRects(line + i, segPrefix.length, segPrefix.length + seg.length) : [];
              if (rrs.length) {
                for (const rr of rrs) mkMark(rr.top, rr.x0 - 2, Math.max(12, rr.x1 - rr.x0 + 4));
                if (paneX == null) paneX = rrs[0].x0;
                paneY = rrs[rrs.length - 1].top + lh + 3;
              } else {
                const x0 = caretX(segPrefix);
                const x1 = caretX(segPrefix + seg);
                mkMark(padT + (line + i) * lh, x0 - 2, Math.max(12, x1 - x0 + 4));
                if (paneX == null) paneX = x0;
                paneY = padT + (line + i + 1) * lh + 3;
              }
              segPrefix = '';
            });
            // the change itself: a pane superimposed on the code right under
            // the struck-out region, showing the COMPLETE replacement —
            // approve or reject in place, everything readable, nothing to open
            const pane = document.createElement('div');
            pane.className = 'sugPane' + (fresh ? ' pulse' : '');
            pane.style.left = `${Math.max(6, Math.min(paneX ?? padL, ed.clientWidth - 560))}px`;
            pane.style.top = `${paneY ?? padT + (line + segs.length) * lh + 3}px`;
            pane.innerHTML = `
            ...
            pane.querySelector('.sugApply').addEventListener('click', (e) => { e.stopPropagation(); applySug(s); });
            pane.querySelector('.sugDismiss').addEventListener('click', (e) => { e.stopPropagation(); resolveSug(key, s.id, 'dismissed'); });
            sugLayer.appendChild(pane);
          }
        };
        sugRedraw[key] = renderSugs; // resolution patches decorations in place
      }
    ...
    ed.addEventListener('input', () => {
      ...
      editorChrome(key);
      hlSchedule();
      // markdown: the rendered pane tracks the draft live (debounced)
      if (ed.dataset.ext === 'md' || ed.dataset.ext === 'markdown') {
        mdPreviewSchedule(key, ed.dataset.rel);
      }
    });
    // LaTeX smarts (completion popup, auto-pairs, auto-\end, ⌘/, ⌘J) —
    // attached BEFORE the generic keydown handler: with the popup open this
    // module owns Tab/Enter/arrows via stopImmediatePropagation
    if (isTex) {
      texEditorAttach(ed, {
        requestMeta: () => api('GET', `/api/texmeta/${enc(key)}`),
        forwardSearch: (line, col) => texForwardSearch(key, ed.dataset.rel, line, col),
        jumpToLine: (ln) => {
          pendingEdJump = { fkey: k, line: ln, col: 1, at: Date.now() };
          applyEdJump(root, ed);
        },
      });
    }
    ...
    // the save chip saves on click when there's anything to save (the chip
    // element survives editorChrome's innerHTML patches, so wire it once)
    const sc = root.querySelector('#saveState');
    if (sc) sc.addEventListener('click', () => {
      if (drafts[ed.dataset.fkey] != null) saveFile(key, ed.dataset.rel);
    });
    if (isTex) {
      root.querySelector('#texOutlineBtn')?.addEventListener('click', (e) => {
        e.stopPropagation();
        texOutlineMenu(e.currentTarget, ed, (ln) => {
          pendingEdJump = { fkey: k, line: ln, col: 1, at: Date.now() };
          applyEdJump(root, ed);
        });
      });
      root.querySelector('#texSyncBtn')?.addEventListener('click', () => {
        ...
        texForwardSearch(key, ed.dataset.rel, line, upto.length - (upto.lastIndexOf('\n') + 1) + 1);
      });
    }
    applyEdJump(root, ed); // an inverse-sync / problems-click landing here
  }
