...
const eligibilityLabels = {
  paused: 'Paused after a worker failure. Review the diagnostic before explicitly resuming.',
  busy: 'A task turn is running; memory waits until it finishes.',
  budget: 'The shared daily memory limit is reached; no request will be sent. Resume does not refund reservations.',
  disabled: 'Automatic task memory is disabled in Settings.',
  'billing-blocked': 'Memory requests are blocked in this test environment.',
  changed: 'The transcript changed before the checkpoint boundary. Review the previous checkpoint before continuing.',
  unavailable: 'The memory connection or selected model is unavailable. Review Settings before another attempt.',
};

// Render only the deliberately bounded diagnostic vocabulary. Never stringify
// a transport object: arbitrary stderr, messages, prompts or credentials do not
// belong in the inspection panel, even if a future server includes them.
function diagnosticsHtml(diagnostics) {
  if (!diagnostics || typeof diagnostics !== 'object') return '';
  const labels = { code: 'code', eventType: 'event', itemType: 'item', stage: 'stage', exitCode: 'exit', version: 'worker version',
    warningCount: 'warnings', warningEventType: 'warning event', warningItemType: 'warning item' };
  const parts = Object.entries(labels).flatMap(([key, label]) => {
    const value = diagnostics[key];
    return typeof value === 'string' || typeof value === 'number'
      ? [`${label}: ${esc(String(value).slice(0, 120))}`] : [];
  });
  return parts.length ? `<p class="memoryNote memoryDiagnostics">${parts.join(' · ')}</p>` : '';
}

function budgetHtml(budget) {
  if (!budget || budget.dailyJobLimit == null) return '';
  const reset = typeof budget.resetAt === 'string' ? new Date(budget.resetAt) : null;
  const resetText = reset && Number.isFinite(reset.getTime())
    ? `${reset.toLocaleString([], { dateStyle: 'medium', timeStyle: 'short' })} (your time)` : 'not supplied';
  return `<p class="memoryNote memorySharedBudget"><b>Shared Codex memory allowance:</b> ${count(budget.jobsToday)} / ${count(budget.dailyJobLimit)} jobs reserved today · ${count(budget.remainingJobs)} remaining across all tasks. UTC-day reset: ${esc(resetText)}.</p>`;
}

function pauseHtml(pause, buttonId) {
  if (!pause) return '';
  return `<div class="memoryNotice memoryPause"><b>Memory updates paused.</b> ${esc(pause.reason || 'The worker needs review before another attempt.')}
    ${diagnosticsHtml(pause.diagnostics)}${pause.persistenceWarning ? `<p class="memoryError">${esc(pause.persistenceWarning)}</p>` : ''}<p>Review the failure before resuming. Resume clears this connection's pause only; it does not start a job, enable memory, refund reservations, or increase the daily limit.</p>
    <button type="button" class="gbtn" id="${buttonId}">Resume memory updates</button></div>`;
}

function reservationLabel(job) {
  if (job.reservationHeld === false) return 'No daily slot reserved';
  return job.configuration?.connection === 'codex-subscription'
    ? '1 subscription job reserved' : `${dollars(job.reservedUsd)} reserved`;
}
...
export async function mountMemorySettings(host) {
  ...
  box.innerHTML = `<h3>Task memory <span class="memoryBadge">Checkpoint-only pilot</span></h3>
    ...
    <form id="memorySettingsForm" class="memoryForm">
      ...
      <div class="memoryActions"><button class="gbtn" type="submit">Save memory settings</button><span role="status">${s.enabled ? 'Enabled' : 'Disabled'}${s.pause ? ' · paused' : ''}</span></div>
    </form>
    ${budgetHtml(s)}${pauseHtml(s.pause, 'memorySettingsResume')}
  ...
  box.querySelector('#memorySettingsResume')?.addEventListener('click', async event => {
    const button = /** @type {HTMLButtonElement} */ (event.currentTarget);
    button.disabled = true;
    const result = await api('POST', '/api/memory/resume', { connection: s.pause.connection });
    if (result) await mountMemorySettings(host);
...
export async function openTaskMemory(project, id, title) {
  ...
  async function refresh() {
    ...
    // A retained queue row can be waiting for explicit action after resume.
    // It is not a dispatched worker and must not lock the Update button.
    const inProgress = data.status === 'running';
    const eligible = data.eligibility ? data.eligibility.eligible === true
      : !inProgress && !data.pause && data.settings.enabled && !data.settings.billingBlocked && data.pending;
    const successes = data.counts?.successful ?? data.revisions.length;
    const failures = data.counts?.failed ?? data.jobs.filter(j => j.status === 'failed').length;
    const blocked = data.counts?.blocked ?? data.jobs.filter(j => j.status === 'blocked').length;
    ...
    panel.innerHTML = `<div class="memoryNotice"><b>Inspection only.</b> This checkpoint is not being read by the working agent. Validation checks structure and source IDs, not factual accuracy.</div>
      <div class="memoryActions"><span class="memoryBadge">${data.pause ? 'paused' : data.status === 'queued' && !inProgress ? 'waiting — no request running' : esc(data.status)}</span><button class="gbtn" id="memoryRefresh">Refresh</button>
      <button class="gbtn" id="memoryUpdate"${!eligible || inProgress || data.pause || !data.pending || !data.settings.enabled || data.settings.billingBlocked ? ' disabled' : ''}>Update checkpoint</button></div>
      <p class="memoryNote memoryOutcomeCounts"><b>${count(successes)} successful checkpoints</b> · ${count(failures)} failed attempts · ${count(blocked)} blocked attempts${data.counts ? '' : ' (attempt counts shown for recent history only)'}. A queued or failed attempt is not a checkpoint.</p>
      ${data.eligibility ? `<p class="memoryNote memoryEligibility">${data.eligibility.eligible ? data.pending ? 'Ready for one bounded checkpoint update.' : 'No new transcript material needs an update.' : esc(eligibilityLabels[data.eligibility.reason] || 'Memory is not eligible for another update. Review its settings and current status.')}</p>` : ''}
      ${budgetHtml(data.budget)}${pauseHtml(data.pause, 'memoryResume')}
      ...
      ${data.coverage ? `<p class="memoryNote memoryCoverage">Coverage: ${count(data.coverage.coveredEvents)} / ${count(data.coverage.totalEvents)} complete transcript entries${data.coverage.partialEvent ? ` + ${count(data.coverage.partialEvent)} characters of the next entry` : ''}. ${data.coverage.totalEvents === 0 ? 'No transcript entries available.' : data.coverage.complete ? 'Available transcript covered.' : 'Unread material remains; this is not a fully current summary.'}</p>` : ''}
      <div class="memoryStats"><div><b>${memoryTokens(data.totals.inputTokens, accountingComplete)}</b><span>reported input tokens</span></div><div><b>${memoryTokens(data.totals.outputTokens, accountingComplete)}</b><span>reported output tokens</span></div><div><b>${dollars(data.totals.estimatedCostUsd)}</b><span>known estimated subtotal; subscription / unknown costs excluded</span></div><div><b>${data.jobs.length}</b><span>recent attempt records</span></div></div>
    ...
    if (inProgress || (!data.pause && data.status === 'queued' && eligible)) timer = setTimeout(refresh, 1500);
  }
  await refresh();
}
